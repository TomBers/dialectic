    ...
    socket =
      ...
      |> then(fn s ->
        cond do
          ...
          true ->
            assign(s, :placeholder, "Ask a follow-up question or share your thinking...")
        end
      end)
    ...
    <div class="w-full min-w-0" data-role="ask-form-container">
      ...
      >
        ...
        <div class="flex items-center gap-2 w-full">
          ...
          <div class="relative min-w-0 flex-1">
            <.input
              ...
              id={@input_id}
              type="textarea"
              label={if(@embedded, do: "What would you like to ask?")}
            ...
            >
              ...
              <% else %>
                <.link
                  ...
                  href={~p"/users/log_in"}
                  class="block rounded-lg bg-indigo-50 px-3 py-2 text-xs font-semibold text-indigo-700"
                >
                  Sign in to add a personal learning plan. Your draft is kept in this tab.
                </.link>
                <button
                  :if={@context == :node}
        ...
        <div
          :if={@show_tools}
          id={"#{@id}-tools-toolbar"}
          class="inquiry-tools-toolbar mb-2 mt-3 grid min-w-0 max-w-full grid-cols-2 items-stretch gap-1.5 border-t border-slate-100 pb-1 pt-3"
        >
          {render_slot(@quick_actions)}
        </div>
        <div :if={@show_tools} id={"#{@id}-more-tools-row"} class="mb-2 flex">
          <button
            id={@tools_button_id}
            type="button"
            ...
            class={[
              ...
              if(@embedded, do: "min-h-11 px-1.5", else: "h-8 px-2.5"),
              if(@tools_open,
          ...
          >
            ...
            >
              <.icon name="hero-plus" class="h-4 w-4" /> More tools
            </span>
            <span
              data-tools-open
              class={["inline-flex items-center gap-1", !@tools_open && "hidden"]}
