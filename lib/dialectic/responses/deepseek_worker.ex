defmodule Dialectic.Workers.DeepSeekWorker do
  ...
  def handle_result(
    ...
    Logger.error("DeepSeek API error: #{inspect(error)}")

    # Format a user-friendly error message
    user_message = "Error from DeepSeek API: #{message} (#{error_type})"
  ...
  def handle_result(other, _graph, _to_node, _live_view_topic) do
    Logger.error("Unhandled DeepSeek response: #{inspect(other)}")
    :ok
  end
