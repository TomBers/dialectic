defmodule Dialectic.Graph.GraphActions do
  ...
  @thinking_tools [
    ...
    {:analogy, "analogy", :gen_analogy, "Generate helpful analogies to explain the concept"},
    ...
    {:simplify, "simplify", :gen_simplify, "Simplify complex ideas into plain language"},
    {:second_order, "second_order", :gen_second_order,
     "Analyze second-order effects and indirect consequences"}
  ]
  ...
  def apply_thinking_tool(tool_key, {graph_id, node, user, live_view_topic}, opts \\ []) do
    {^tool_key, class, llm_fn, _doc} =
      Enum.find(@thinking_tools, fn {key, _class, _llm_fn, _doc} -> key == tool_key end)
  ...
  defp validate_regeneration(class, parents) do
    ...
    cond do
      ...
      class in ["thesis", "antithesis", "ideas", "answer"] or class in thinking_tool_classes ->
